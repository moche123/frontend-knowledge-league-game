...
@Component({
  ...
  template: `
    <div class="flex border-b border-outline-variant" [class]="variant() === 'underline' ? 'gap-8' : ''">
      @for (tab of tabs(); track tab.id) {
        <button
          type="button"
          (click)="active.set(tab.id)"
          class="pb-3 font-title-sm text-title-sm transition-colors duration-300 border-b-2"
          [class]="variant() === 'segmented' ? 'flex-1 text-center font-label-caps text-label-caps' : ''"
